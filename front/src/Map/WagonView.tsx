...
import type {Dislocation, SetMapLocation, TimeEventWagon, WagonEventsParsed, WagonTimelineParsed} from '../interface';
    ...
    React.useEffect(() => {
        ...
        getWagonTimeLine(id, moment)
            .then((data) => {
                ...
                const bounds = nextTimeline.events.reduce<LngLatBounds>(
                    (memo, event) => {
    ...
    return (
        <>
            {timeline.events.map((event, index) => (
                <WagonMarker key={index} event={event} wagon={timeline.vagon} order={index + 1} />
            ))}
            <ymaps.YMapFeature
